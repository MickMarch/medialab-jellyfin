...
class TestLibraryScan:
    def test_scan_returns_204(self, client):
        with patch("medialab_jellyfin.services.jellyfin.requests.post", return_value=_jellyfin_204()):
            response = client.post(SCAN_URL, json={"path": "/data/movies/Foo (2024)", "update_type": "Created"})

        assert response.status_code == 204

    def test_scan_forwards_correct_payload_to_jellyfin(self, client):
        with patch("medialab_jellyfin.services.jellyfin.requests.post", return_value=_jellyfin_204()) as mock_post:
            client.post(SCAN_URL, json={"path": "/data/movies/Foo (2024)", "update_type": "Modified"})
    ...
    def test_scan_rejects_invalid_update_type(self, client):
        response = client.post(SCAN_URL, json={"path": "/data/movies/Foo (2024)", "update_type": "Invalid"})
    ...
    def test_scan_returns_503_when_jellyfin_unavailable(self, client):
        with patch("medialab_jellyfin.services.jellyfin.requests.post", return_value=_jellyfin_503()):
...
class TestLibraryPaths:
    def test_add_path_returns_204(self, client):
        with patch("medialab_jellyfin.services.jellyfin.requests.get", return_value=_jellyfin_virtual_folders(VIRTUAL_FOLDERS_MOVIES)), \
             patch("medialab_jellyfin.services.jellyfin.requests.post", return_value=_jellyfin_204()):
            response = client.post(PATHS_URL, json={"media_type": "movie", "path": "/data/movies/Foo (2024)"})

        assert response.status_code == 204

    def test_add_path_resolves_movie_library(self, client):
        with patch("medialab_jellyfin.services.jellyfin.requests.get", return_value=_jellyfin_virtual_folders(VIRTUAL_FOLDERS_MOVIES)), \
             patch("medialab_jellyfin.services.jellyfin.requests.post", return_value=_jellyfin_204()) as mock_post:
    ...
    def test_add_path_refresh_library_param_forwarded(self, client):
        with patch("medialab_jellyfin.services.jellyfin.requests.get", return_value=_jellyfin_virtual_folders(VIRTUAL_FOLDERS_MOVIES)), \
             patch("medialab_jellyfin.services.jellyfin.requests.post", return_value=_jellyfin_204()) as mock_post:
            client.post(PATHS_URL, json={"media_type": "movie", "path": "/data/movies/Foo (2024)", "refresh_library": True})
    ...
    def test_add_path_library_name_override_skips_discovery(self, client):
        with patch("medialab_jellyfin.services.jellyfin.requests.get") as mock_get, \
             patch("medialab_jellyfin.services.jellyfin.requests.post", return_value=_jellyfin_204()) as mock_post:
            client.post(PATHS_URL, json={"media_type": "movie", "path": "/data/movies/Foo (2024)", "library_name": "My Films"})
    ...
    def test_add_path_returns_404_when_no_matching_library(self, client):
        ...
        with patch("medialab_jellyfin.services.jellyfin.requests.get", return_value=_jellyfin_virtual_folders(folders)):
            response = client.post(PATHS_URL, json={"media_type": "movie", "path": "/data/movies/Foo (2024)"})
    ...
    def test_add_path_returns_409_when_ambiguous_library(self, client):
        with patch("medialab_jellyfin.services.jellyfin.requests.get", return_value=_jellyfin_virtual_folders(VIRTUAL_FOLDERS_AMBIGUOUS)):
            response = client.post(PATHS_URL, json={"media_type": "movie", "path": "/data/movies/Foo (2024)"})
    ...
    def test_add_path_returns_503_when_jellyfin_unavailable(self, client):
        with patch("medialab_jellyfin.services.jellyfin.requests.get", return_value=_jellyfin_503()):
            response = client.post(PATHS_URL, json={"media_type": "movie", "path": "/data/movies/Foo (2024)"})
...
class TestLibraryItems:
    def test_items_returns_200_with_results(self, client):
        ...
        with patch("medialab_jellyfin.services.jellyfin.requests.get", return_value=_jellyfin_items_response(items, 1)):
    ...
    def test_items_forwards_search_term(self, client):
        with patch("medialab_jellyfin.services.jellyfin.requests.get", return_value=_jellyfin_items_response([], 0)) as mock_get:
    ...
    def test_items_includes_request_id_header(self, client):
        with patch("medialab_jellyfin.services.jellyfin.requests.get", return_value=_jellyfin_items_response([], 0)):
            response = client.get(ITEMS_URL)
